...
export async function updateHospitalAvailability(
  ...
  if (!Number.isNaN(backendId)) {
    try {
      ...
      if (patch.icu?.available !== undefined) payload.icu_available = patch.icu.available;
      if (patch.general?.available !== undefined) {
        payload.general_available = patch.general.available;
      }
      if (patch.ventilators?.available !== undefined) {
        payload.ventilators_available = patch.ventilators.available;
      }
